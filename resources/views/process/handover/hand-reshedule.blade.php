...
@section('content')
    ...
    @can('order-reschedule.mark')
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-primary text-white action-header">
                        <p>Mark as Rescheduled delivery </p>
                    </div>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select name="" id="reason_id" class="form-control js-example-basic-single">
                                    ...
                                    @foreach ($reschedule_reason as $reason)
                                        <option value="{{ $reason->reschedule_reason_id }}">{{ $reason->reason }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <button type="button" class="btn btn-primary btn-block" id="btn-submit"> Mark as Re-Schedule</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @if ($user->can('order-reschedule.search') && $user->can('order-reschedule.view'))
        <div class="form-row">
            <div class="col-md-12">
                <div class="card">
    ...
    @can('order-reschedule.view')
        <div class="row">
            <div class=" col-md-12">
                <div class="card">
                    <div class="card-header bg-secondary text-white">
                        <p>Rescheduled Packages </p>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered display" id="table-data" style="width:100%">
                                <thead>
                                    <tr>
                                        <th scope="col">Final Re-Schedule Date</th>
        ...
        <script>
            ...
            function load_data(from_date, to_date, branch_id, seller_id, staff_id) {
                $('#table-data').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '{{ route('hand_reschedule_orders_data_table') }}',
                        data: {
                            from_date: from_date,
                            to_date: to_date,
                            branch_id: branch_id,
            ...
            function reschedule() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        notify('success', 'Package Re-Schedule');
                        $('#table-data').DataTable().destroy();
                        load_data();

                        setTimeout(function() {
                            $('#btn-submit').html('Mark as Re-Schedule');
                        }, 2000);
